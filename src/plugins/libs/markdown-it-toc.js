...
module.exports = function(md, opt) {
    ...
    md.renderer.rules.heading_open = function(tokens, index) {
        ...
        if (label.type === 'inline') {
            ...
            return '<' + level + '><a id="' + (options.resetIds ? anchor : label.content) + '"></a>';
        } else {
            return '</h1>';
        }
    };
    ...
    md.renderer.rules.toc_body = function(tokens, index) {
        ...
        var list = headings.map(function(heading) {
            ...
            res = res.concat(['<li><a href="#', (options.resetIds ? heading.anchor : heading.content), '">', heading.content, '</a></li>']);
            return res.join('');
        });

        if (tokens[index].content) {
            return '<div class="toc-block"><h3>' + tokens[index].content + '</h3>' + list.join('') + new Array(indent + 1).join('</ul>') + "</div>";
        } else {
            return '<div class="toc-block">' + list.join('') + new Array(indent + 1).join('</ul>') + "</div>";
        }
    };
