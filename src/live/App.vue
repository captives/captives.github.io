    ...
    <template v-if="logined">
      ...
      <el-aside width="400px">
        ...
        <el-tabs type="border-card" class="online-list" v-model="activeName">
          ...
          <el-tab-pane label="座次表" name="first">
            <OnlineList :data="tableData"></OnlineList>
          </el-tab-pane>
...
import CoursePanel from './views/CoursePanel';
...
export default {
  ...
  methods: {
    ...mapActions("subject", ["startSubject", "stopSubject"]),
    logoutHandler() {
      this.client.close();
    },
    loginComplete(user) {
  ...
  mounted() {
    ...
    this.axios.get('/surfing/index.json').then(({ data }) => {
      this.courseList = data.list.map(item => { return '/surfing/' + item });
    });
