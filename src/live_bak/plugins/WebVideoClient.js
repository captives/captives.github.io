        ...
        if (devices && devices.length > 0) {
          devices.forEach((device) => {
            ...
            if (device["kind"] == "videoinput") {
              device.label =
                device.label || "Camera " + (that.cameras.length + 1);
              that.cameras.push(device);
            } else if (device["kind"] == "audioinput") {
              device.label =
                device.label || "Microphone " + (that.microphones.length + 1);
              that.microphones.push(device);
            } else if (device["kind"] == "audiooutput") {
              device.label =
                device.label || "Audio " + (that.audios.length + 1);
              that.audios.push(device);
            }
          });
          resolve({
            videoinput: that.cameras,
            audioinput: that.microphones,
            audiooutput: that.audios,
          });
        } else {
