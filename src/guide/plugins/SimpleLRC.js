...
Plugin.prototype.init = function () {
  if (this.settings.el) this.settings.wrapper = this.settings.el;
  var that = this;
  this.getLrc(this.settings.url, function (lrc) {
...
Plugin.prototype.initDOM = function () {
  ...
  this.settings.wrapper.appendChild(child);
...
Plugin.prototype.start = function () {
  ...
  this.interval = setInterval(function () {
    var currentTime = that.settings.currentPlayTime() - that.settings.delay;
    ...
    if (currentLine > halfRow) {
      that.ul.style.top =
        (halfRow - currentLine) * that.settings.lineHeight + "px";
    }
  }, this.settings.freq);
  ...
  function findCurrentLine(lrc, currentTime, start) {
    ...
    for (var i = start; i < lrc.length; i++) {
      if (
        currentTime >= lrc[i][0] &&
        (!lrc[i + 1] || (lrc[i + 1] && currentTime < lrc[i + 1][0]))
      ) {
        return i;
      }
    }
