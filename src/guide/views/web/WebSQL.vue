<template>
  <el-main>
    ...
    <p>
      在数据库中处理大量结构化数据，html5引入Web SQL Database概念，它使用 SQL
      来操纵客户端数据库的 API，这些 API
      是异步的，规范中使用的是SQLlite（SQL后端）。
    </p>
    <h4>三个核心方法</h4>
    <ul>
      <li>
    ...
    <el-row>
      ...
      <vue-code>
        <pre lang="javascript">
var dataBase = window.openDatabase(database_name, database_version, database_displayname, database_size);</pre
        >
      </vue-code>
      ...
      <el-form :inline="true" :model="dataBase" label-width="60px">
        ...
        <el-form-item>
          <el-button
            ...
            >断开数据库</el-button
          >
          <el-button v-else type="primary" @click="connectHandler"
            >连接数据库</el-button
          >
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    insertHandler() {
      const that = this;
      const sql = `INSERT INTO demos (id, name, address) VALUES (${Date.now()}, ${
        this.table.name
      }, ${this.table.address})`;
      this.db.transaction((tx) => {
        tx.executeSql(sql);
