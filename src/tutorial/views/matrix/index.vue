<template>
  <el-main>
    ...
    <h3>CSS3中的矩阵</h3>
    <p>Matrix的中文是矩阵，在计算机科学中，会用矩阵来对向量进行变换，在css3的transform属性中，可以使用矩阵对图像进行变换。</p>
    <p>CSS3中的矩阵指的是一个方法，书写为matrix()和matrix3d()，前者是元素2D平面的移动变换(transform)，后者则是3D变换。2D变换矩阵为3x3；3D变换则是4x4的矩阵。</p>
    <p>transform的matrix()方法格式如下:</p>
    <vue-code>
      <pre lang="js">matrix(a: number, b: number, c: number, d: number, x: number, y: number)</pre>
    </vue-code>
    <p>参数对应矩阵的格式为</p>
    <p>
      <img :src="images.matrix1" alt />
    </p>
    <p>第一行代表的是X轴变化，第二行代表的是Y轴的变化，第三行代表的是Z轴的变化，2D不涉及到Z轴，这里使用 0 0 1;</p>
    <p>对应规则大致为:</p>
    <ul>
    ...
    <p>
      矩阵相乘的规则
      <b>左一行右一列,相乘再相加</b>,
      <em>ax+cy+e</em>为变换后的水平坐标，
      <em>bx+dy+f</em>
      表示变换后的垂直位置
    </p>

    <h3>缩放 scale(x, y)</h3>
    <p>默认值(1, 1), 缩放对应的是矩阵中的a和d，x轴的缩放比例对应a，y轴的缩放比例对应d。</p>
    <vue-code>
      <pre lang="css">transform: scale(1.5, 2);
//等同于
transform: matrix(1.5, 0, 0, 2, 0, 0);</pre>
    ...
    <vue-code>
      <pre lang="css">transform: translate(300, 150);
//等同于
transform: matrix(1, 0, 0, 1, 300, 150);</pre>
    ...
    <vue-code>
      <pre lang="css">transform: rotate(30);
//等同于
const r = π / 180 × θ;
transform: matrix(cos(r), sin(r), -sin(r), cos(r), 0, 0);</pre>
    ...
    <vue-code>
      <pre lang="css">// 弧度和角度的转换公式：r = π / 180 × θ 
...
const cos = Math.cos(radian); // 计算 cosθ</pre>
    ...
    <p>默认值(0, 0), 扭曲/斜切对应的是矩阵中的c和 b。x的斜切对应c, y的斜切对应b。 这个值对应并不是相等，需要对skew的x值和y值进行tan运算。</p>
    <vue-code>
      <pre lang="css">transform: skew(20deg, 30deg);
//等同于
...
transform: matrix(1, tan30, tan20, 1, 0, 0);</pre>
    ...
    <h3>复合使用</h3>
    <p>
      在实际使用中,我们会复合使用这些, 那么我们就需要将缩放、平移、旋转、斜切等按照
    ...
    <p>
      把方阵A
      <sub>2x4</sub>的行变为新方阵B
      <sub>4x2</sub>列的形式, B就是A的转置矩阵;
    </p>
    <p>
    ...
    <el-drawer
      title="CSS3 transform 中的 matrix"
