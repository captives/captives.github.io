<template>
  <el-popover popper-class="vue-popove" trigger="click">
    ...
    <ul>
      <li v-for="(item, index) in browser" :key="index">
        <template v-if="item instanceof Object">
          {{ index }} :
          <span v-for="(child, idx) in item" :key="idx" class="li-child"
            >{{ idx }} : {{ child }}</span
          >
        </template>
    ...
    <el-button
      slot="reference"
      :type="browser['online'] ? '' : 'danger'"
      icon="el-icon-s-platform"
      >环境({{ env.NODE_ENV }})</el-button
    >
  </el-popover>
</template>
...
export default {
  ...
  methods: {
    ...
    getBrowserInfo() {
      ...
      if (e && e.timing) {
        ...
        if (t.domComplete && t.domLoading) {
          if (t.domContentLoadedEventStart && t.domLoading) {
            info["domComplete(domLoaded)"] =
              t.domContentLoadedEventStart - t.domLoading + "ms";
          } else {
      ...
      if (e.memory) {
        info["(内存)使用的JS堆"] =
          ((e.memory.usedJSHeapSize / e.memory.totalJSHeapSize) * 100).toFixed(
            2
          ) + "%";
        info["(内存)限制的JS堆"] =
          ((e.memory.usedJSHeapSize / e.memory.jsHeapSizeLimit) * 100).toFixed(
            2
          ) + "%";
      }
    ...
    animate() {
      var e =
        window.performance || window.msPerformance || window.webkitPerformance;
      if (e.memory) {
        this.loadInfo["(内存)使用的JS堆"] =
          ((e.memory.usedJSHeapSize / e.memory.totalJSHeapSize) * 100).toFixed(
            2
          ) + "%";
        this.loadInfo["(内存)限制的JS堆"] =
          ((e.memory.usedJSHeapSize / e.memory.jsHeapSizeLimit) * 100).toFixed(
            2
          ) + "%";
