  ...
  <el-dialog title="编辑" :visible.sync="data.visible" width="910px" append-to-body>
    <el-row class="cropper-box">
      ...
      <div class="slider-view">
        ...
        <p>原始尺寸：{{previews.img && previews.img.width | pxToVal}}x{{previews.img && previews.img.height | pxToVal}}</p>
        <p>裁剪尺寸：{{Math.round(previews.w * option.enlarge ) }}x{{Math.round(previews.h * option.enlarge)}}</p>
        <p>输出尺寸：{{Math.round(previews.w * option.enlarge )}}x{{Math.round(previews.h * option.enlarge)}}</p>
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button type="danger" v-if="data.file" @click="submitHandler(false)">原 图</el-button>
    </span>
  </el-dialog>
</template>
<script lang="ts">
...
export default class ImageCropper extends Vue {
  ...
  private imgLoad(msg: any) {
    ...
    let width: number = Number(data.width.replace("px", ""));
    let height: number = Number(data.height.replace("px", ""));
  ...
  private submitHandler(operation: boolean) {
    this.data.visible = false;
    if (operation) {
      let file: File = new window.File([this.blob], this.data.name || "cut—image", { type: this.blob.type });
