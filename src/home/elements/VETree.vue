<template>
  <el-main>
    ...
    <el-tree :data="data" node-key="id" default-expand-all :expand-on-click-node="false">
      <span class="custom-tree-node" slot-scope="{ node, data }" @dblclick="edit(node, data)">
        <el-input
          v-if="node.edited"
          placeholder="请输入内容"
          v-model="data.label"
          size="mini"
          clearable
          @change="rename(node, data)"
        >
          <el-button slot="append" icon="el-icon-circle-check" @click="rename(node, data)">确认</el-button>
        </el-input>
