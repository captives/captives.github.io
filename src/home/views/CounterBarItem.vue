<template>
  <li>
    ...
    <span ref="currentBottom" class="current stage-icon-bottom">{{
      value
    }}</span>
  ...
  methods: {
    run() {
      ...
      var topTween = new Tween(property)
        .to({ rotateX: -90 }, 200)
        .onUpdate(function () {
          oldTop.style.setProperty(
            "transform",
            "rotateX(" + property.rotateX + "deg)"
          );
      ...
      var bottomTween = new Tween(property)
        ...
        .to({ rotateX: -90 }, 200)
        .onUpdate(function () {
          currentBottom.style.setProperty(
            "transform",
            "rotateX(" + (90 + property.rotateX) + "deg)"
          );
